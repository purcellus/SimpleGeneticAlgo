...
#define MAX_GEN         20   // GA stops after this many generations
...
/*********************************************************
   Here is were we will evaluate our function.  d is considered the x range.
*********************************************************/
double evaluate(int value)
{
...
double func(double x){
  return (double)abs((x*sin(x*.5))/(cos(x)*cos(x)+1))*.01*x*x;
}

double func2(double x){
  return (double)x*sin(x) - 0.3*x;
}
...
double convRange(int raw)
{
  double outval = ((((double)raw)/1048576.0)*400)-200;
  //TODO see if you can change this to a bit shift for CHROM_LENGTH
  return(outval);
}
